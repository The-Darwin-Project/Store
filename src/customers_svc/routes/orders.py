# @ai-rules:
# 1. [Cross-service]: Stock deduction and coupon operations go through InventoryClient HTTP calls.
# 2. [Atomicity]: If stock deduction fails mid-order, previously deducted items are restored.
# 3. [Gotcha]: Dashboard reads use shared DB directly (acceptable for logical split).
"""Order endpoints for Customer Management Service."""
...
async def create_order(order_data: OrderCreate, request: Request) -> Order:
    """Create a new order. Uses inventory service for stock deduction and coupon validation."""
    pool = request.app.state.db_pool
    inventory: InventoryClient = request.app.state.inventory_client
    conn = pool.getconn()
    deducted_items = []  # Track for rollback
    try:
        with conn.cursor() as cur:
            ...
            for item in order_data.items:
                # Cross-service: deduct stock via inventory service
                try:
                    result = await inventory.deduct_stock(item.product_id, item.quantity)
                except InventoryError as e:
                    # Rollback previously deducted items
                    for deducted in deducted_items:
                        try:
                            await inventory.restore_stock(deducted["product_id"], deducted["quantity"])
                        except Exception:
                            logger.error(f"Failed to restore stock for {deducted['product_id']}")
                    raise HTTPException(status_code=e.status_code, detail=e.detail)

                deducted_items.append({"product_id": item.product_id, "quantity": item.quantity})
                price_at_purchase = result["price_at_purchase"]
            ...
            if order_data.coupon_code:
                try:
                    coupon_result = await inventory.validate_coupon(order_data.coupon_code, total_amount)
                except InventoryError as e:
                ...
                if not coupon_result.get("valid"):
                    ...
                    for deducted in deducted_items:
                        try:
                            ...
                            logger.error(f"Failed to restore stock for {deducted['product_id']}")
                    raise HTTPException(status_code=400, detail=coupon_result.get("error", "Coupon validation failed"))
                ...
                try:
                    await inventory.increment_coupon_usage(coupon_result["coupon"]["id"])
                except InventoryError as e:
                    # Rollback stock deductions
                    for deducted in deducted_items:
                        try:
        ...
        conn.rollback()
        # Rollback stock deductions on unexpected error
        for deducted in deducted_items:
            try:
                await inventory.restore_stock(deducted["product_id"], deducted["quantity"])
            except Exception:
...
async def update_order_status(order_id: str, body: OrderStatusUpdate, request: Request) -> Order:
    """Update the status of an order. Restores stock via inventory service on cancel/return."""
    pool = request.app.state.db_pool
    inventory: InventoryClient = request.app.state.inventory_client
    conn = pool.getconn()
    try:
        with conn.cursor() as cur:
            ...
            # Restore stock via inventory service when cancelling or returning
            if body.status in (OrderStatus.CANCELLED, OrderStatus.RETURNED):
                ...
                for product_id, quantity in items:
                    try:
                        await inventory.restore_stock(str(product_id), quantity)
                    except InventoryError as e:
                        logger.error(f"Failed to restore stock for product {product_id}: {e.detail}")
...
async def generate_invoice(order_id: str, request: Request) -> Invoice:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Fetch order items joined with products (shared DB read)
            cur.execute(
