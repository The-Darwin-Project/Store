      ...
      <ModalBody id="product-detail-content">
        <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          <div>
            ...
            {avgRating && avgRating.review_count > 0 && (
              <div style={{ marginTop: '0.5rem' }}>
                {renderStars(Math.round(avgRating.average_rating))}
                <span style={{ marginLeft: '0.5rem' }}>
                  {avgRating.average_rating.toFixed(1)} ({avgRating.review_count} review{avgRating.review_count !== 1 ? 's' : ''})
                </span>
              </div>
            )}
            {product.stock > 0 && (
