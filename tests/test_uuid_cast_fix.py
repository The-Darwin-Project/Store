...
def test_get_orders_with_uuid_objects(mock_pool_cls):
    ...
    mock_cursor.fetchall.side_effect = [
        ...
        [
            (order_id_1, now, 50.0, "confirmed"),
            (order_id_2, now, 30.0, "confirmed"),
        ],
    ...
    order_items_query = sql_calls[1]  # second execute call
...
def test_get_orders_passes_string_ids_to_query(mock_pool_cls):
    ...
    mock_cursor.fetchall.side_effect = [
        [(order_id, datetime.datetime.now(), 10.0, "confirmed")],
        [],  # no items
    ]
